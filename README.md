...
0. If you see files that you didn't write, consider moving the files into one of the [paths for vendored  code](/lib/linguist/vendor.yml), or use the [manual overrides](#overrides) feature to override them.
...
All plain text files are true source files. Generated files like minified js and compiled CoffeeScript can be detected and included from language stats. As an added bonus, link vendored and documentation files, these files are suppressed in diffs.
...
All other files are covered by the MIT license, see `License.md`.
